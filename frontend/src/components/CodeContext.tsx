...
export default function CodeContext({ onSelectedFilesChange }) {
  ...
  useEffect(() => {
    const getSelectedFiles = (items: FileItem[]): string => {
      return items.reduce((acc, item) => {
        if (item.isSelected && !item.isDirectory) {
          acc += `File: ${item.path}\n${item.content}\n\n`;
        }
        if (item.children) {
          acc += getSelectedFiles(item.children);
        }
        return acc;
      }, "");
    };

    const selectedFilesContent = getSelectedFiles(files);
    onSelectedFilesChange(selectedFilesContent);
